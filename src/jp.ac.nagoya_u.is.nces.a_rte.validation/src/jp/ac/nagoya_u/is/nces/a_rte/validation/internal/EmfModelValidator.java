...
public class EmfModelValidator implements IModelValidator {
	...
	public boolean validate(Resource eResource, DiagnosticChain diagnostics) {
		BasicDiagnostic emfDiagnostics = new BasicDiagnostic();

		Diagnostician diagnostician = createDiagnostician();
		for (EObject eObject : eResource.getContents()) {
			diagnostician.validate(eObject, emfDiagnostics);
		}

		boolean isValid = emfDiagnostics.getSeverity() != Diagnostic.ERROR;
		
		diagnostics.addAll(emfDiagnostics);
		return isValid;
	}

	private Diagnostician createDiagnostician() {
		return new Diagnostician() {
			@Override
			public String getObjectLabel(EObject eObject) {
				return ModelLabels.getLabel(eObject);
			}
		};
	}
}
