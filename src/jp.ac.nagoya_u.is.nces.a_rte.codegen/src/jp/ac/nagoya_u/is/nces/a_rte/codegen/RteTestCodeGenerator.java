...
import jp.ac.nagoya_u.is.nces.a_rte.codegen.rte_test.main.RteTestCodes;
...
public class RteTestCodeGenerator {
	private final List<CodeFormatter> codeFormatters = Lists.newArrayList();

	public static RteTestCodeGenerator forGeneratePhase() {
		return new RteTestCodeGenerator();
	}

	private RteTestCodeGenerator() {
		...
		try {
			...
			rteTestCodes.addGenerationListener(new AbstractAcceleoTextGenerationListener() {
				...
				public void fileGenerated(AcceleoTextGenerationEvent event) {
					...
					try {
						for (CodeFormatter codeFormatter : RteTestCodeGenerator.this.codeFormatters) {
			...
			rteTestCodes.doGenerate(new BasicMonitor());
