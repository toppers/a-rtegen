...
#
#  Copyright (C) 2013-2022 by Center for Embedded Computing Systems
...
#  の責任を負わない．
#
# $Id: common.sh 822 2017-03-15 07:20:08Z mtakada $
#
...
CAN_TARGET_PATH=$COMSTACK_PATH/can/target/$TARGET

# A-COMSTACKで必要なモジュール
MODULE="$MODULE Com.o Com_PBcfg.o CanIf.o CanIf_PBcfg.o CanIf_Lcfg.o Can.o Can_Irq.o Can_PBcfg.o Can_Prc.o"
...
define_perf()
{
	MODULE="$MODULE histogram.o measure_swc$1.o"
...
generate_code()
{
	...
	perl $OS_PATH/configure -T $TARGET -A Rte $CFG_OPT \
		-a "$COM_PATH $PDUR_PATH $CANIF_PATH $CAN_PATH $CAN_PATH/arch/gcc $CAN_PATH/arch/rs_can $COM_STUB_PATH $COM_GENERAL_PATH $CAN_TARGET_PATH $GENERAL_PATH/EcuM $GENERAL_PATH/HelloAutosar $INCLUDE ../stub" \
		-C $APPLICATION \
		-U "$MODULE C_Init_Code.o EcuM.o EcuM_StartupTask.o Os_Hook.o" "$CMP_OPT"

	# ABREXによるARXMLの作成
	ruby $OS_PATH/utils/abrex/abrex.rb ../../$APPLICATION.yaml ../hsbrh850f1k_gcc_common.yaml
	mv -f ../../$APPLICATION.arxml .

	# A-RTEGENによるA-RTEモジュール作成
	bash $RTE_PATH/bin/bin/rtegen.sh $OS_PATH/target/$TARGET/target_hw_counter.arxml $GENERAL_PATH/HelloAutosar/$SYSTEM_DESC.arxml $APPLICATION.arxml
	...
	echo "Generate Can"
	$OS_PATH/cfg/cfg/cfg --omit-symbol --pass 2 --kernel atk2 --ini-file $CAN_PATH/can.ini --api-table $CAN_PATH/can.csv -I $CAN_PATH -I $CAN_PATH/arch/rs_can -T $CAN_PATH/target/$TARGET/Can_Target.tf $APPLICATION.arxml $OS_PATH/target/$TARGET/target_hw_counter.arxml
}
